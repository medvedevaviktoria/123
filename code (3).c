...
namespace EventPlanner
{
    public partial class MainForm : Form
    {
        ...
        private void addEventButton_Click(object sender, EventArgs e)
        {
            ...
            {
                errors.Add("Неверный формат ФИО (Фамилия Имя Отчество/Фамилия Имя).");
                contactNameTextBox.BackColor = Color.Red;
            }
       ...
       // Очистка полей ввода
        ...
        {
           //  Проверяет, что строка содержит только буквы (русские и латинские), пробелы и дефисы, и что есть хотя бы одно слово, затем еще одно через пробел
             return Regex.IsMatch(contactName, @"^[a-zA-Zа-яА-Я]+(\s[a-zA-Zа-яА-Я]+)*$");
        }
